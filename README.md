...
### Failed Alignments ###
In some cases, LRCstats may fail to align untrimmed corrected long reads back to the Ref-uLR alignment. This occurs when there exists an uncorrected segment in the cLR which is not completely identical to the corresponding segment in the uLR. Occurrences of these non-identical uncorrected segments in the cLR are the fault of correction algorithm, so we do not include these faulty reads into the statistics calculations.
